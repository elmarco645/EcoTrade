        ...
        <div className="flex items-center justify-between">
          ...
          {activeFilterCount > 0 && (
            <button 
              ...
              className="group flex items-center gap-1 text-xs font-bold text-blue-600 transition-all hover:text-blue-700"
            >
          ...
          <label className="flex cursor-pointer items-center gap-3">
            <div className="relative flex h-5 w-5 items-center justify-center">
              <input
                ...
                className="peer h-full w-full appearance-none rounded border border-slate-300 bg-white checked:bg-blue-600 checked:border-blue-600 transition-all"
              />
      ...
      <div className="flex-1 space-y-8">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            ...
            <p className="text-slate-500">Browse {filteredListings.length} items from our community.</p>
          </div>
          <div className="flex w-full items-center gap-4 md:w-auto">
            <div className="relative flex-1 md:w-80">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              />
            </div>
            <button 
              onClick={() => setShowFilters(true)}
              className="relative flex h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 font-medium text-slate-600 hover:bg-slate-50 lg:hidden"
            >
          ...
          {showFilters && (
            <>
              ...
              >
                ...
                <div className="h-[calc(100vh-12rem)] overflow-y-auto space-y-8 pr-2">
                  {/* Reuse filter sections logic here for mobile drawer if needed, or just scrollable list */}
                  {/* Category */}
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Category</h3>
                      ...
                      {CATEGORIES.map(cat => (
                        <button
                          ...
                          className={`rounded-lg border px-3 py-2 text-xs font-medium transition-colors ${
                            filters.category === cat ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-slate-200 text-slate-600'
                          }`}
                        >
                          {cat}
                        </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CONDITIONS.map(cond => (
                        <button
                          ...
                          onClick={() => setFilters(f => ({ ...f, condition: f.condition === cond ? '' : cond }))}
                          className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                            filters.condition === cond ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'
                          }`}
                        >
                          {cond}
                        </button>
                ...
                >
                  <div className="relative aspect-square overflow-hidden rounded-2xl bg-slate-100">
                    ...
                    {listing.is_negotiable === 1 && (
                      <div className="absolute right-3 top-3 rounded-full bg-emerald-500/90 px-3 py-1 text-[10px] font-bold text-white backdrop-blur-sm uppercase tracking-tighter">
                        Negotiable
                      </div>
                    )}
                  </div>
